...
namespace geometry
{
    ...
    std::vector<Vector> visibility_polygon2(
        Vector point,
        InputIterator begin,
        InputIterator end)
    {
        ...
        for (auto&& event : events)
        {
            ...
            else if (cmp_dist(event.segment, *state.begin()))
            {
                ...
                auto adad = ray.intersects(nearest_segment, intersection);
                if (!adad) {
                    int t = 3;
                }
                //auto intersects = ray.intersects(nearest_segment, intersection);
                vec2f o = vec2f(ray.origin.x, ray.origin.y);
                vec2f dir = vec2f(normalize(ray.direction).x, normalize(ray.direction).y);
                vec2f p1 = vec2f(nearest_segment.a.x, nearest_segment.a.y);
                vec2f p2 = vec2f(nearest_segment.b.x, nearest_segment.b.y);
                float intersects = geom::ray_line_intersect(&o, &dir, &p1, &p2);


                assert(intersects >= 0.0f &&
