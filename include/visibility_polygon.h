...
struct slice {
	...
	slice(std::vector<vec2f*>* _pts, std::vector<vec2f*>* _canonical_pts, float heading) {
		for (int i = 0; i < _pts->size(); i++){
		//for (vec2f* v : *_pts) {
			pts.push_back(new vec2f((*_pts)[i]->x, (*_pts)[i]->y));
			canonical_pts.push_back(new vec2f((*_canonical_pts)[i]->x, (*_canonical_pts)[i]->y));
		}
		...
		canonical_p1 = canonical_pts[0];
		canonical_p2 = canonical_pts.back();
		target_pt = (*canonical_p1 + *canonical_p2) * 0.5f;
	...
	std::vector<vec2f*> canonical_pts;
	vec2f* p1;
	vec2f* p2;
	vec2f* canonical_p1;
	vec2f* canonical_p2;
