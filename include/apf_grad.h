...
class apf_grad : public redirector{
	...
	private:
		...
		redirection_unit set_gains(float dx, float dy, float dtheta, trajectory_unit cur_move, float phys_heading);
		...
		vec2f steer_target;
		vec2f gradient_dir;
		std::vector<std::vector<vec2f>> gradient_data;
