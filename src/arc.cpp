 ...
 */
float arc::attenuate_gain(float angle_to_target, float cur_gain) {
...
float arc::get_distance_in_direction(vec2f pos, vec2f dir, environment* env) {
	float closest = math::max_float;
...
vec2f arc::get_optimal_heading(vec2f pos, visibility_polygon vis_poly, float dist_north, float dist_east, float dist_south, float dist_west) {
