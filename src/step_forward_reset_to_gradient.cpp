...
step_forward_reset_to_gradient::step_forward_reset_to_gradient() {
    name = "step forward reset to gradient";
...
float step_forward_reset_to_gradient::reset(simulation_state& sim_state, user* egocentric_user) {
    ...
    vec2f dummy_pos = phys_pos;
    for (int i = 0; i < SFR2G_MAX_STEPS; i++) {
        gradient_dir = compute_gradient(phys_env, prox_queue, dummy_pos);
        dummy_pos += gradient_dir * SFR2G_STEP_SIZE;
    }
    return reorient_to_target(phys_heading, virt_heading, phys_pos, virt_pos, dummy_pos, path);
}

vec2f step_forward_reset_to_gradient::compute_gradient(physical_environment* phys_env, std::deque<proximity_container*> prox_queue, vec2f phys_pos) {
...
float step_forward_reset_to_gradient::compute_repulsive_force(vec2f pos, std::vector<object*> objects) {
    float dist_sum = 0.0f;
